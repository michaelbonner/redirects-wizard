<template>
    <div 
        class="bg-white mx-auto shadow-md rounded overflow-hidden mb-4"
    >
        <div class="p-4 border-gray border-b mb-6" v-if="dev_url">
			<button 
            ...
            <button 
                ...
                :class="{ 'cursor-not-allowed': submittingRecheckUnaddressed, 'opacity-50': submittingRecheckUnaddressed }"
            >
                <i 
            ...
            <button 
                ...
                :class="{ 'cursor-not-allowed': submittingRecheckAll, 'opacity-50': submittingRecheckAll }"
            >
                <i 
                    class="fas fa-sync-alt"
			...
			<a 
                :href="rewriteUrl" 
                target="_blank"
                class="float-right no-underline text-sm font-semibold rounded px-4 py-1 leading-normal text-blue mx-1"
            >
                View Rewrites
                ...
                <div class="mb-8" v-if="!validUrl">
                    <div class="bg-blue-lightest border-t-4 border-blue rounded-b text-blue-darkest px-4 py-3 shadow-md" role="alert">
                        <div class="flex">
                            ...
                            <div>
                                <p class="font-bold">You must enter a valid dev url before you can start</p>
                                <p class="text-sm">The dev URL is where we are actively developing the site. Generally this will be https://{yoursite}.redolive.co</p>
                                <p class="text-sm">Be sure to include the https:// part</p>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="mb-8">
                    <p class="text-sm leading-tight text-grey-dark">{{ urlCount }} URLs</p>
                </div>
        ...
        <div
            class="float-right p-4 text-right"
        >
            <button
                class="text-red-darker no-underline"
    ...
    export default {
        ...
        methods: {
            ...
            updateDevUrl: function () {
                ...
                this.error = {};
                this.success = '';
                axios.patch(`/api/batch/${this.batch_id}`, {
            ...
            archive: function() {
                axios.delete(`/api/batch/${this.batch_id}`)
                    .then((data) => {
                        alert('Archived');
