<template>
    <div>
        <h2 class="text-2xl text-grey-darkest mt-8 mb-4 ml-4">
            Add New URLs to Test
        </h2>
        <div
            class="bg-white mx-auto overflow-hidden mb-4"
        >
            <form 
                class="w-full my-8 px-4"
            ...
            >
                ...
                <div class="md:flex md:items-center">
                    ...
                    <div class="md:w-4/3">
                        <button 
                            class="shadow bg-blue hover:bg-blue-light focus:shadow-outline focus:outline-none text-white font-bold py-2 px-4 rounded" 
                            type="submit"
                            v-bind:disabled="submitting"
                        >
                            {{ submitText }}
            ...
            onSubmit: function (event) {
                if(this.uniqueUrls.length){
                    ...
                    for (let i = 0; i < this.uniqueUrls.length; i++) {
                        const url = this.uniqueUrls[i].substring(0, 4) === 'http' ? this.uniqueUrls[i] : `http://${this.uniqueUrls[i]}`;
                        promises.push(
                            axios.post(`/api/batch/${this.batch_id}/url`, {
                                url
                            })
                            .then(() => {
                                this.countAdded++;
