...
const localDatabaseUrl = envValues.DATABASE_URL;
...
if (productionDatabaseUrl === localDatabaseUrl) {
    throw new Error("PRODUCTION_DATABASE_URL and DATABASE_URL must be different.");
}

const production = parsePostgresConnection(productionDatabaseUrl);
const local = parsePostgresConnection(localDatabaseUrl);
...
try {
    ...
    console.log(`Ensuring local database exists: ${local.redactedUrl}`);
    ensureDatabaseExists(local);

    console.log(`Restoring dump into local database: ${local.redactedUrl}`);
    run(
        "pg_restore",
        [
            "--clean",
    ...
    console.log("Copied production database to local.");
} finally {
